...
export class ListeRecetteComponent {
  ...
  getDatar() {
    this.recommendation = window.location.href;

    const test = this.recommendation.split('/');
    if (test) {
      this.recommendation = test[3];

      console.log(this.recommendation);
      if (this.recommendation === 'Recommendation') {
        this.recommendation = 'Recommendation';
      }
    }
    if (this.recommendation === 'Recommendation') {
      // Appeler la fonction du service pour récupérer les données
      this.messageService.getDataRecommender().subscribe((data: any) => {
